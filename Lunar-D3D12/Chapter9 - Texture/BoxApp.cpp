...
void BoxApp::Draw(const GameTimer& gt)
{
	...
	address.ptr += mCbvSrvUavDescriptorSize;
	...
	mCommandList->SetGraphicsRootDescriptorTable(3, address);
...
void BoxApp::BuildConstantBuffers() {
	...
	D3D12_CPU_DESCRIPTOR_HANDLE handle{ cbvHeap->GetCPUDescriptorHandleForHeapStart() };
	handle.ptr += mCbvSrvUavDescriptorSize;

	md3dDevice->CreateShaderResourceView(mTextures["Wood"].get()->Resource.Get(), &srvDesc, handle);

	Material m{};
	m.DiffuseAlbedo = { 1.0f, 1.0f, 1.0f, 1.0f };
	m.DiffuseSrvHeapIndex = 0;
	m.Roughness = 0.25f;
	m.MatCBIndex = 0;

	mMaterials["box"] = std::make_unique<Material>(m);
}
