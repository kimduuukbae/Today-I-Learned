...
namespace Vector3 {
	...
	inline XMFLOAT3 ScalarProduct(const XMFLOAT3& f3Vector, float fScalar, bool bNormalize = true) {
		XMFLOAT3 result{};
		if (bNormalize)
			XMStoreFloat3(&result, XMVector3Normalize(XMLoadFloat3(&f3Vector) * fScalar));
		else
			XMStoreFloat3(&result, XMLoadFloat3(&f3Vector) * fScalar);
		return result;
	}
	...
	inline float Angle(const XMFLOAT3& xmvVector1, const XMFLOAT3& xmvVector2) {
		XMVECTOR xmvAngle = XMVector3AngleBetweenNormals(XMLoadFloat3(&xmvVector1), XMLoadFloat3(&xmvVector2));
		return(XMConvertToDegrees(acosf(XMVectorGetX(xmvAngle))));
	}
