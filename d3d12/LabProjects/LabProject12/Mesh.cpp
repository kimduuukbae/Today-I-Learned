...
void CMesh::Render(ID3D12GraphicsCommandList* commandList, UINT nInstances) {
	commandList->IASetPrimitiveTopology(primTopology);
	if (indexBuffer) {
		commandList->IASetIndexBuffer(&indexBufferView);
		commandList->DrawIndexedInstanced(indices, nInstances, 0, baseVertex, startIndex);
	}
	else
		commandList->DrawInstanced(vertices, nInstances, offset, 0);
}

void CMesh::Render(ID3D12GraphicsCommandList* commandList, UINT nInstances, D3D12_VERTEX_BUFFER_VIEW instancingBufferView){
	// 버텍스 버퍼 뷰랑 인스턴싱 버퍼 뷰를 IA 단계에 설정
	D3D12_VERTEX_BUFFER_VIEW pVertexBufferViews[] = { vertexBufferView, instancingBufferView };
	commandList->IASetVertexBuffers(slot, _countof(pVertexBufferViews), pVertexBufferViews);
	Render(commandList, nInstances);
}
...
CAirplaneMeshDiffused::CAirplaneMeshDiffused(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,
	...
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(-fx, -y3, -fz), Vector4::Add(xmf4Color,
		RANDOM_COLOR));
	vertexBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, pVertices,
		stride * vertices, D3D12_HEAP_TYPE_DEFAULT,
		D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &vertexUploadBuffer);
	vertexBufferView.BufferLocation = vertexBuffer->GetGPUVirtualAddress();
	vertexBufferView.StrideInBytes = stride;
