...
void CGameObject::Render(ID3D12GraphicsCommandList* commandList, CCamera* pCamera, UINT nInstances, D3D12_VERTEX_BUFFER_VIEW instancingBufferView){
	OnPrepareRender();

	if (mesh) mesh->Render(commandList, nInstances, instancingBufferView);
}
