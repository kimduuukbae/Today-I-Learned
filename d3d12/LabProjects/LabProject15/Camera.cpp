...
void CCamera::GenerateFrustum(){
	frustum.CreateFromMatrix(frustum, XMLoadFloat4x4(&projectMatrix));
	XMMATRIX inversView{ XMMatrixInverse(nullptr, XMLoadFloat4x4(&viewMatrix)) };
	frustum.Transform(frustum, inversView);
}
bool CCamera::IsInFrustum(BoundingOrientedBox& boundBox){
	return frustum.Intersects(boundBox);
}
